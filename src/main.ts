...
var prefix = !!process.env.AZURE_HTTP_USER_AGENT
  ...
  : "";
var azPSHostEnv = !!process.env.AZUREPS_HOST_ENVIRONMENT
...
async function main() {
  try {
    ...
    const enableAzPSSession =
      core.getInput("enable-AzPSSession").toLowerCase() === "true";
    const allowNoSubscriptionsLogin =
    ...
    if (servicePrincipalId || tenantId || subscriptionId) {
      ...
      if (
        !(
          servicePrincipalId &&
          tenantId &&
          (subscriptionId || allowNoSubscriptionsLogin)
        )
      )
        throw new Error(
          "Few credentials are missing. ClientId, tenantId are mandatory. SubscriptionId is also mandatory if allow-no-subscriptions is not set."
        );
    } else {
      if (creds) {
        ...
        resourceManagerEndpointUrl = secrets.getSecret(
    ...
    if (
      !servicePrincipalId ||
      !tenantId ||
      !(servicePrincipalKey || enableOIDC)
    ) {
      throw new Error(
        "Not all values are present in the credentials. Ensure clientId, clientSecret and tenantId are supplied."
      );
    }
    if (!subscriptionId && !allowNoSubscriptionsLogin) {
      throw new Error(
        "Not all values are present in the credentials. Ensure subscriptionId is supplied."
      );
    }
    if (!azureSupportedCloudName.has(environment)) {
      throw new Error(
        "Unsupported value for environment is passed.The list of supported values for environment are ‘azureusgovernment', ‘azurechinacloud’, ‘azuregermancloud’, ‘azurecloud’ or ’azurestack’"
      );
    }
      ...
      try {
        ...
        if (!!federatedToken) {
          if (environment != "azurecloud")
            throw new Error(
              `Your current environment - "${environment}" is not supported for OIDC login.`
            );
    ...
    if (environment == "azurestack") {
      ...
      try {
        ...
        await executeAzCliCommand(
          `cloud unregister -n "${environment}"`,
          false
        );
      } catch (error) {
      ...
      console.log(
        `Registering cloud: "${environment}" with ARM endpoint: "${resourceManagerEndpointUrl}"`
      );
      try {
        ...
        await executeAzCliCommand(
          `cloud register -n "${environment}" --endpoint-resource-manager "${resourceManagerEndpointUrl}" --suffix-keyvault-dns "${suffixKeyvault}" --suffix-storage-endpoint "${suffixStorage}" --profile "${profileVersion}"`,
          false
        );
      } catch (error) {
        core.error(
          `Error while trying to register cloud "${environment}": "${error}"`
        );
      }
    ...
    if (!allowNoSubscriptionsLogin) {
      var args = ["--subscription", subscriptionId];
    ...
    if (!isAzCLISuccess) {
      ...
      core.setFailed(
        `Azure PowerShell Login failed. Please check the credentials and make sure az is installed on the runner. For more information refer https://aka.ms/create-secrets-for-GitHub-workflows"`
      );
    }
  } finally {
